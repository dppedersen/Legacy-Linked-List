...
module.exports = function(app, express) {
  ...
  app.get("/api/users", function(req, res) {
    ...
    if (username.google.id !== "") {
      User.findOne({ "google.id": username.google.id }).exec(function(
        err,
        user
      ) {
        if (err) {
      ...
      User.findOne({ "local.username": username.local.username }).exec(function(
        err,
        user
      ) {
        if (err) {
  ...
  app.delete("/api/users", function(req, res) {
    ...
    if (username.google.id !== "") {
      User.remove({
        "google.username": username.google.username
      }).exec((err, user) => {
        if (err) {
          ...
          if (err) {
            ...
            username.jobs.forEach(job => {
              var event = {
                ...
                attendees: [{ email: username.google.email }]
              };
    ...
    if (username.google.id !== "") {
      User.findOne({ "google.id": username.google.id }).exec((err, user) => {
        if (err) {
          ...
          res.send(
            companies.filter((val, index) => companies.indexOf(val) === index)
          );
        }
      });
    } else {
    ...
    if (username.google.id !== "") {
      User.findOne({ "google.id": username.google.id }).exec((err, user) => {
        if (err) {
          ...
          if (google.email !== "") {
            var options = {
              url: `https://www.googleapis.com/calendar/v3/calendars/${username
                .google.email}/events?maxResults=2500`,
              method: "GET",
              headers: {
                "User-Agent": "request",
  ...
  app.get("/api/companyInfo", function(req, res) {
    ...
    rp(options)
      ...
      .catch(function(err) {
        res.status(400).send("Somethings wrong, please try again!");
      });
  });
